...
```{r}
df <- read.csv("/Users/sbp/Downloads/NewHeadlinesV2.csv")

corpus <- df$Headline
```
```{r}
#read the data
tf <- "/Users/sbp/Downloads/testtransvv2.csv"
trans <- file(tf)
...
for(i in 2:nrow(df)){
  tokens <- tokenizers::tokenize_words(df$Headline[i], stopwords = stopwords::stopwords("en"), lowercase = TRUE, strip_punct = TRUE, strip_numeric = TRUE, simplify = TRUE)
  cat(unlist(str_squish(tokens)), "\n", file = trans, sep = ",")
}
close(trans)
```
...
TweetTrans_rules <- arules::apriori(TweetDf,
                                    parameter = list(support = 0.03, confidence = 0.1))
